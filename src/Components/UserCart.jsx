...
const UserCart = () => {
  ...
  async function fetchCart() {
    ...
    try {
      const res = await axios.get(
        `https://readify-fdkn.onrender.com/cart?username=${loggedUser.username}`
      );
      setCart(res.data);
    } catch (err) {
      console.log(err);
  ...
  const increase = async (item) => {
    ...
    await axios.patch(`https://readify-fdkn.onrender.com/cart/${item.id}`, {
  ...
  const removeItem = async (id) => {
    await axios.delete(`https://readify-fdkn.onrender.com/cart/${id}`);
    fetchCart();
  };
  const clearCart = async () => {
    try {
      ...
      for (const item of res.data) {
        await axios.delete(`https://readify-fdkn.onrender.com/cart/${item.id}`);
      }
  ...
  const placeOrder = async () => {
    try {
      ...
      for (const item of cartItems) {
        ...
        // Save order
        await axios.post("https://readify-fdkn.onrender.com/orders", {
          ...item,
        ...
        const bookRes = await axios.get(
          `https://readify-fdkn.onrender.com/books/${item.bookId}`
        );
        ...
        // Remove from cart
        await axios.delete(
          `https://readify-fdkn.onrender.com/cart/${item.id}`
        );
      }
